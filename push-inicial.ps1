...
if ([string]::IsNullOrWhiteSpace($status)) {
    ...
    if ($LASTEXITCODE -ne 0) {
        ...
        git commit -m "feat: Renomeia para GelaFit Control e implementa monitoramento de app

- Renomeado de MRIT Control para GelaFit Control
- Novo serviço AppMonitorService para monitorar app escolhido
- Serviços sempre rodam em background mesmo quando app fechado
- Melhorado modo kiosk para prevenir fechamento automático
- Garante que GelaFit Control inicia quando app escolhido abre
- Capacidade de reiniciar app escolhido remotamente via comando
- Todos os serviços reiniciam automaticamente quando destruídos
...
try {
    ...
    Write-Host "🔗 Acesse: https://github.com/MRITSoftware/kiosk-gelafit/actions" -ForegroundColor Cyan
} catch {
    Write-Host ""
    ...
    Write-Host "   - Ou configure SSH: git remote set-url origin git@github.com:MRITSoftware/kiosk-gelafit.git" -ForegroundColor White
    exit 1
}
